...
from app.services.investigations import get_asset
...
def _run_drift(payload: DriftRequest, connection: sqlite3.Connection, settings: Settings, direction: int) -> DriftResponse:
    ...
    return DriftResponse(direction="forward" if direction == 1 else "backward", seed={"latitude": payload.latitude, "longitude": payload.longitude, "observed_at": payload.observed_at.isoformat(), "duration_hours": payload.duration_hours, "particle_count": payload.particle_count}, **result)
...
def rank_vessels(payload: AttributionRequest, connection: DatabaseConnection, settings: AppSettings) -> AttributionResponse:
    ...
    return AttributionResponse(disclaimer="Candidate ranking is an explainable investigative lead based on uploaded evidence. It does not establish legal guilt or responsibility.", candidate_count=len(candidates), scoring_formula=SCORE_WEIGHTS, candidates=candidates)
